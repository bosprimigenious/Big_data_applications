...
class ResearchReportGenerator:
    ...
    def add_section(self, title, content, level=1):
        """添加报告章节"""
        prefix = "#" * level
        self.report_content.append(f"{prefix} {title}\n")
        self.report_content.append(content + "\n\n")
    
    def generate_research_direction(self):
        """生成研究方向部分"""
        content = """
## 1. 研究背景与意义
...
- 为法律部门提供证据链完整性保障
        """
        self.add_section("研究方向与理论基础", content)
...
5. **结果分析**：深入分析模型性能和错误案例
        """
        self.add_section("技术实现与实验设计", content)
    
    def generate_results_analysis(self, results_data=None):
        """生成结果分析部分"""
        content = """
## 7. 实验结果与分析
...
- 结合多模态信息（如写作时间、来源等）
        """
        self.add_section("实验结果与深度分析", content)
...
4. 查看帮助：`python main.py --help`
        """
        self.add_section("结论、局限性与展望", content)
    
    def save_report(self, filename="research_report.md"):
        """保存报告到文件"""
        # 添加报告头部信息
        header = f"""# Human vs AI Generated Essays 研究报告
...
"""
    ...
    def generate_summary_report(self, filename="executive_summary.md"):
        """生成执行摘要"""
        summary = f"""# 执行摘要
...
def main():
    ...
    # 生成各个部分
    generator.generate_research_direction()
    generator.generate_methodology()
