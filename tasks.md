...
| ？？？ | 9-12 | 实验结果（上）：基础指标 + 可视化 | 1. 模型评估表（准确率 / F1-score）；<br>2. 混淆矩阵热力图；<br>3. 错误案例分析（AI 文本被误判为人类的特征）；<br>4. 特征重要性排序（哪些词对分类贡献大）。 | 屈子元提供模型结果；许佳琪协助核对指标计算 |
...
| ？？？ | 19-20 | 总结 + 致谢 | 1. 核心结论（模型效果 / 关键发现）；<br>2. 学习收获（非代码成员的 NLP 入门心得）；<br>3. 致谢（指导老师 + 组员）。 | 许佳琪提供整合结论；全体成员提供学习感悟 |
### 制作要求
